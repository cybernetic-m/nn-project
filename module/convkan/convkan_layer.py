...
class ConvKAN(torch.nn.Module):
    def __init__(
        ...
        """
        Convolutional layer with KAN kernels. A drop-in replacement for torch.nn.Conv1d.
        ...
        Args:
            ...
            bias (bool): Added for compatibility with torch.nn.Conv1d and does make any effect. Default: True
        ...
        self._in_dim = (in_channels // groups) * self.kernel_size[0]
    ...
    def forward(self, x):
        ...
        # Apply 1D convolution using unfold
        x_unf = F.unfold(
            x.unsqueeze(2),  # Add a dimension for the height (required for unfold)
        ...
        x_unf = (
            x_unf.permute(0, 2, 1)  # [B, L_out, channels * elems]
            .reshape(
                batch_size * n_patches, self.groups, channels_and_elem // self.groups
            )  # [B * L_out, groups, out_channels // groups]
            .permute(1, 0, 2)
        )  # [groups, B * L_out, out_channels // groups]
